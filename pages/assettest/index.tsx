    ...
    try {
      if (newAsset) {
        if (data.abrand && data.modelnum) {
          // Create an array of selected part IDs
          const selectedPartIds = selectedParts.map((partId) => partId);
  
          await fetch('api/assetcreate', {
            body: JSON.stringify({ ...data, partid: selectedPartIds }), // Include the selected part IDs
            headers: {
...
const updatedData = {
  // ... (other properties)
  partBrand: selectedParts.map((partId) => {
    const selectedPart = parts.find((part) => part.id === partId);
    return selectedPart ? selectedPart.brand : '';
  }),
  partIdp: selectedParts.join(', '), // Concatenate selected part IDs
};
...
{/* 
                ...
                <select
                  ...
                  <label htmlFor="partid" className="select_label">
                    Part
                  </label>
                  <MultiSelect
                    id="partid"
                    ...
                    className='bg-black-900 border border-gray-300 text-gray-50 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block mx-auto w-80
                    p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500'
                  />
                  ...
                  {parts.map((part) => (
                    <div className="text-white" key={part.idp}>
                      <label>
                        <input
                          type="checkbox"
                          value={part.idp}
                          checked={selectedParts.includes(part.idp)}
                          onChange={() => handlePartSelection(part.idp)}
                        />
                        {part.brand} ({part.idp})
                      </label>
                    </div>
